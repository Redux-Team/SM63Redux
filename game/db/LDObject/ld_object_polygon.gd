...
var outer: LDPolygon = LDPolygon.new()
...
func get_outer_points() -> PackedVector2Array:
	return outer.to_flat()


func get_holes() -> Array[PackedVector2Array]:
	var result: Array[PackedVector2Array] = []
	for h: LDPolygon in holes:
		result.append(h.to_flat())
	return result
...
func get_hole(index: int) -> PackedVector2Array:
	if index < holes.size():
		return holes[index].to_flat()
	return PackedVector2Array()


func apply_segments(new_outer: LDPolygon, new_holes: Array[LDPolygon]) -> void:
	outer = new_outer
	holes = new_holes
	_rebuild_draw()


func apply_points_raw(points: PackedVector2Array, raw_holes: Array[PackedVector2Array]) -> void:
	outer = LDPolygon.from_flat(points)
	holes.clear()
	for h: PackedVector2Array in raw_holes:
		if h.size() >= 3:
			holes.append(LDPolygon.from_flat(h))
	_rebuild_draw()
...
func remove_hole(index: int) -> void:
	if index >= 0 and index < holes.size():
		holes.remove_at(index)
		_rebuild_draw()


func _rebuild_draw() -> void:
	var outer_flat: PackedVector2Array = outer.to_flat()
	...
	for h: LDPolygon in holes:
		var hf: PackedVector2Array = h.to_flat()
		if hf.size() < 3:
			continue
...
func _update_visuals() -> void:
	...
	for h: LDPolygon in holes:
		var hole_ccw: PackedVector2Array = TerrainPolygon.ensure_counter_clockwise(h.to_flat())
	...
	if _outline_container:
		...
		for h: LDPolygon in holes:
			...
			hole_line.points = TerrainPolygon.subdivide_for_line2d(
				TerrainPolygon.reverse_points(TerrainPolygon.get_closed_points(
					TerrainPolygon.ensure_clockwise(h.to_flat()))), outline_tex)
			_outline_container.add_child(hole_line)


func _draw() -> void:
	var draw_points: PackedVector2Array = outer.to_flat()
	...
	if is_preview:
		...
		if draw_points.size() >= 2:
			draw_polyline(
				TerrainPolygon.get_closed_points(draw_points) if draw_points.size() >= 3 else draw_points,
				p_border, PREVIEW_BORDER_WIDTH, true)
		for h: LDPolygon in holes:
			var hf: PackedVector2Array = h.to_flat()
			if hf.size() >= 3:
				draw_colored_polygon(hf, Color(0.0, 0.0, 0.0, 0.4))
				draw_polyline(TerrainPolygon.get_closed_points(hf), p_border, PREVIEW_BORDER_WIDTH, true)
		return
	if not (terrain_data and terrain_data.base_texture):
		draw_colored_polygon(draw_points, FALLBACK_FILL)
		draw_polyline(TerrainPolygon.get_closed_points(draw_points), FALLBACK_BORDER,
			terrain_data.border_width if terrain_data else 3.0, true)
		for h: LDPolygon in holes:
			var hf: PackedVector2Array = h.to_flat()
			if hf.size() >= 3:
				draw_colored_polygon(hf, Color(0.0, 0.0, 0.0, 0.5))
				draw_polyline(TerrainPolygon.get_closed_points(hf), FALLBACK_BORDER,
					terrain_data.border_width if terrain_data else 3.0, true)
...
func _get_polygon_bounds() -> Rect2:
	var flat: PackedVector2Array = outer.to_flat()
	if flat.is_empty():
		return Rect2()
	var bounds: Rect2 = Rect2(flat[0], Vector2.ZERO)
	for point: Vector2 in flat:
