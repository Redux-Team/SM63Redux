...
@export var dive_y_to_time_curve: Curve
@export var dive_y_curve_min: float = -300.0
@export var dive_y_curve_max: float = 300.0
...
func get_air_rotation_angle() -> float:
	...
	var rotation_curve_max: float = dive_rotation_curve.max_domain
	
	if dive_rotation_curve:
		rotation_curve_min = dive_rotation_curve.min_domain
		rotation_curve_max = dive_rotation_curve.max_domain
	...
	if dive_rotation_curve:
		curve_value = dive_rotation_curve.sample(rotation_time)
	else:
...
func update_dive_rotation(delta: float) -> void:
	...
	var visual_angle = rad_to_deg(body_rotation)
	if player.sprite.flip_h:
...
func get_rotation_time_offset_from_velocity(y_vel: float) -> float:
	if dive_y_to_time_curve:
		var min_d: float = dive_y_to_time_curve.min_domain
		var max_d: float = dive_y_to_time_curve.max_domain
		var clamped_y: float = clamp(y_vel, min_d, max_d)
		return dive_y_to_time_curve.sample(clamped_y)
	
	# fallback: linear map from configured min/max to 0..dive_rotation_curve.max_domain
	var clamped_y2: float = clamp(y_vel, dive_y_curve_min, dive_y_curve_max)
	var norm: float = inverse_lerp(dive_y_curve_min, dive_y_curve_max, clamped_y2)
	return norm * dive_rotation_curve.max_domain
