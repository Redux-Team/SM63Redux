...
const FLUDD_DIVE_VERTICAL_ACCEL: float = 3012.0
const FLUDD_DIVE_HORIZONTAL_ACCEL: float = 1000.0
const FLUDD_DIVE_DAMPING_X: float = 0.97
const FLUDD_DIVE_DAMPING_Y: float = 0.98 

var dive_boost: bool = true
...
func _physics_process(delta: float) -> void:
	if Input.is_action_pressed("use_fludd"):
		...
		if dive_boost:
			...
			player.velocity.x += horizontal_accel * facing
		else:
